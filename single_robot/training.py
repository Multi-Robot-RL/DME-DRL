...
def create_environment(houseexpo_dataset, room_id):
    ...
    robot_obstacle_map = np.zeros(map_size)  # Robot has no knowledge of obstacles initially
...
for episode in range(MAX_EPISODES):
    ...
    ground_truth_obstacle_map, frontier_map, robot_obstacle_map = create_environment(dataset, episode % TRAIN_DATASET_SIZE)
    map_size = ground_truth_obstacle_map.shape
    current_location = (np.random.randint(map_size[0]), np.random.randint(map_size[1]))
    ...
    for step in range(MAX_STEPS):
        ...
        if collision:
            ...
            reward = EXPLORATION_REWARD  # Reward for valid movement and exploration
    ...
    performance = evaluate_model_performance(frontier_map, ground_truth_obstacle_map)
    print(
